...
class CustomHeader extends Component {
  ...
  render() {
    ...
    return showSearch ? (
      ...
      <Appbar.Header
        ...
        {this._renderTitle()}
        {this._renderRight()}
        {isHome?(null):this._renderHome()}
        <StatusBar
          backgroundColor={this.props.backgroundColor || Colors.Red900}
          barStyle={!this.props.light ? 'dark-content' : 'light-content'}
        />
      </Appbar.Header>
    );
  ...
  _renderTitle() {
    ...
    return (
      <Appbar.Content
        title={title}
        titleStyle={{color: titleColor || Colors.white, ...styles.titleStyle}}
      />
    );
    // } else if (image) {
  ...
  _renderOption(options) {
    if (options) {
      ...
      if (icon || image) {
        return (
          <Appbar.Action size={16} icon={icon || image} onPress={onPress} />
        );
      } else if (text) {
        return (
          <Clickable
            ...
            onPress={onPress}>
            <Text style={{...styles.textStyle, color: color || Colors.white}}>
              {text}
            </Text>
          </Clickable>
        );
  ...
  _renderHome() {
    return (
        <Appbar.Action size={16} icon={Images.ic_home} style={styles.homeStyle} onPress={()=>{reset("Home")}} />
      );
  }
}
