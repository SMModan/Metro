...
class Contacts extends Component {
  ...
  _renderItem = (item, index, section) => {
    return (
      <Card style={{ margin: 7, marginLeft: 25, marginRight: 25 }} key={index}>
        <View style={{ margin: 15, flexDirection: 'row', alignItems: 'center' }}>
          <View style={{ width: 60, height: 60, borderRadius: 30, backgroundColor: this._getRandomColor(), justifyContent: 'center', marginRight: 15 }}>
  ...
  render() {
    return (
      ...
      <Searchbar placeholder={'Search Contact & Company'} style={{ position: 'absolute', top:0, marginLeft: 50, marginRight: 20, backgroundColor: 'rgba(0, 0, 0, 0.2)',  shadowColor: 'transparent' }} inputStyle={{ color:'white', }} placeholderTextColor={'#8F9BB3'} iconColor={'#8F9BB3'}/>
      </>
    );
  }
}
